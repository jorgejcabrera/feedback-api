...
public interface FeedbackRepository extends PagingAndSortingRepository<Feedback, Long> {
  ...
  @Query(
      value = "select distinct f.store_id from feedback f",
      countQuery = "select distinct count(store_id) from feedback",
      nativeQuery = true)
  Page<String> findAllStoreId(Pageable pageable);
  ...
  @Query(
      "select case when count(f)>0 then true else false end from Feedback f where f.buyerId = ?1")
  boolean existsBuyerId(Long buyer);

  @Query(
      "select case when count(f)>0 then true else false end from Feedback f where f.storeId = ?1")
